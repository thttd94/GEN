...
(function($, window, document, undefined) {
	...
	Picker.prototype.startInertiaScroll = function(event) {
		 ...
		 */
		var nowTime = event.timeStamp || Date.now();
		var v = (point.pageY - self.lastMoveStart) / (nowTime - self.lastMoveTime); //最后一段Giờ间手指划动速度  
		var dir = v > 0 ? -1 : 1; //加速度方向  
		var deceleration = dir * 0.0006 * -1;
		var duration = Math.abs(v / deceleration); // 速度消减至0所需Giờ间  
...
(function($, document) {
	...
	var panelBuffer = '<div class="mui-poppicker">\
		<div class="mui-poppicker-header">\
			<button class="mui-btn mui-poppicker-btn-cancel">Hủy</button>\
	...
	var PopPicker = $.PopPicker = $.Class.extend({
		...
		init: function(options) {
			var self = this;
			self.options = options || {};
			self.options.buttons = self.options.buttons || ['Hủy', '确定'];
...
(function($, document) {
	...
	var domBuffer = '<div class="mui-dtpicker" data-type="datetime">\
		<div class="mui-dtpicker-header">\
			<button data-id="btn-cancel" class="mui-btn">Hủy</button>\
			<button data-id="btn-ok" class="mui-btn mui-btn-blue">确定</button>\
		</div>\
		<div class="mui-dtpicker-title"><h5 data-id="title-y">Năm</h5><h5 data-id="title-m">Tháng</h5><h5 data-id="title-d">Ngày</h5><h5 data-id="title-h">Giờ</h5><h5 data-id="title-i">Phút</h5></div>\
	...
	var DtPicker = $.DtPicker = $.Class.extend({
		...
		_create: function(options) {
			var self = this;
			options = options || {};
			options.labels = options.labels || ['Năm', 'Tháng', 'Ngày', 'Giờ', 'Phút'];
			options.buttons = options.buttons || ['Hủy', '确定'];
